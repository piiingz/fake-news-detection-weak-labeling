...
from snuba_weak_labeling.program_synthesis.synthesizer import Synthesizer
...
class HeuristicGenerator(object):
    ...
    def __init__(self, train_primitive_matrix, val_primitive_matrix,
                 val_ground, train_ground=None, b=0.5, has_snorkel=False, use_mv=False):
        """
    ...
    def apply_heuristics(self, heuristics, primitive_matrix, feat_combos, beta_opt, knn = False):
        ...
        def marginals_to_labels(hf, X, beta):
            if len(hf.classes_) == 1:
                if -1 in hf.classes_:
                    marginals = 1 - hf.predict_proba(X)[0]
                else:
                    marginals = hf.predict_proba(X)[0]
            else:
    ...
    def run_verifier(self):
        ...
        self.vf = Verifier(self.L_train, self.L_val, self.val_ground, has_snorkel=self.has_snorkel, use_mv=self.use_mv)
    ...
    def evaluate(self):
        ...
        def calcualte_f1(marginals, b, ground):
            labels = np.sign(2 * (marginals - 0.5))
            print(labels)
            df = pd.DataFrame({
                'label': labels,
            ...
            non_abstain_labels = df[df['label'] != 0]
            return f1_score(non_abstain_labels['label'], non_abstain_labels['ground'])
        ...
        self.val_f1 = calcualte_f1(self.val_marginals, self.b, self.val_ground)
        self.train_f1 = calcualte_f1(self.train_marginals, self.b, self.train_ground)
        return self.val_accuracy, self.train_accuracy, self.val_coverage, self.train_coverage, self.val_f1, self.train_f1
